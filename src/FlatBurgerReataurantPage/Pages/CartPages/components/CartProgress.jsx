...
export default function CartProgress({ currentStep = 0 }) {
  ...
  const handleStepClick = (index) => {
    if (index === 0) navigate('/FlatBurger/cart');
    if (index === 1) navigate('/FlatBurger/checkout');
    if (index === 2) navigate("/FlatBurger/place-order");
    if (index === 3) navigate('/FlatBurger/confirm-order');
  };
